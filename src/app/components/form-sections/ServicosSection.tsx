...
const ServicosSection: React.FC<Props> = React.memo(
  ({ data, onChange, isReadOnly, isPreposto, activeServico, setActiveServico, onBlur }) => {
    ...
    const updateServico = (servicoKey: string, updates: Partial<ServicoData>) => {
      ...
      const novoServico: ServicoData = {
        horarioInicioManha:
          updates.horarioInicioManha !== undefined
            ? updates.horarioInicioManha
            : servico.horarioInicioManha,
        horarioFimManha:
          updates.horarioFimManha !== undefined ? updates.horarioFimManha : servico.horarioFimManha,
        horarioInicioTarde:
          updates.horarioInicioTarde !== undefined
            ? updates.horarioInicioTarde
            : servico.horarioInicioTarde,
        horarioFimTarde:
          updates.horarioFimTarde !== undefined ? updates.horarioFimTarde : servico.horarioFimTarde,
        local: updates.local !== undefined ? updates.local : servico.local,
        etapas: updates.etapas !== undefined ? { ...updates.etapas } : { ...servico.etapas },
        fotos:
          updates.fotos !== undefined
            ? updates.fotos
              ? [...updates.fotos]
              : undefined
            : servico.fotos
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {/* Manhã - Início */}
                <div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Início</div>
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateServico(servicoKey, { horarioInicioManha: e.target.value })
                      }
                      disabled={isReadOnly}
                ...
                {/* Manhã - Fim */}
                <div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Fim</div>
                  <div className="relative">
                    <Clock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
                    <input
                      type="time"
                      ...
                      onChange={(e) =>
                        updateServico(servicoKey, { horarioFimManha: e.target.value })
                      }
                      disabled={isReadOnly}
                      className="w-full pl-12 pr-4 py-3 rounded-xl 
                               bg-white dark:bg-gray-800 text-gray-900 dark:text-white
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateServico(servicoKey, { horarioInicioTarde: e.target.value })
                      }
                      disabled={isReadOnly}
                      className="w-full pl-12 pr-4 py-3 rounded-xl 
                               bg-white dark:bg-gray-800 text-gray-900 dark:text-white
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateServico(servicoKey, { horarioFimTarde: e.target.value })
                      }
                      disabled={isReadOnly}
                      className="w-full pl-12 pr-4 py-3 rounded-xl 
                               bg-white dark:bg-gray-800 text-gray-900 dark:text-white
                ...
                return (
                  <div key={index}>
                    <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">
                      <span className="text-[#C6CCC2] dark:text-gray-500">{numero}.</span>{' '}
                      {etapa.label}
                    </label>
                    {etapa.isDropdown ? (
                      <div className="flex gap-2">
                        ...
                        >
                          <span
                            className={`truncate ${getDropdownTipo(servicoKey, etapa.label) ? '' : 'text-[#C6CCC2] dark:text-gray-500'}`}
                          >
                            {getDropdownTipo(servicoKey, etapa.label) ||
                              (isReadOnly ? '' : 'Selecione')}
                          </span>
                          ...
                          <span
                            className={
                              getMultiSelectSelectedTypes(servicoKey, etapa.label).length > 0
                                ? ''
                                : 'text-[#C6CCC2] dark:text-gray-500'
                            }
                          >
                            {getMultiSelectSelectedTypes(servicoKey, etapa.label).length > 0
    ...
    return (
      <section>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-700 dark:text-gray-300 mb-6">
                Deseja remover o {confirmDelete.servico === 'servico2' ? 'Serviço 2' : 'Serviço 3'}?
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  type="button"
                  onClick={() =>
                    confirmRemoverServico(confirmDelete.servico as 'servico2' | 'servico3')
                  }
        ...
        {showUretanoMultiSelect.servicoKey &&
          showUretanoMultiSelect.label &&
          (() => {
                ...
                >
                  ...
                  <div className="p-4 space-y-2 overflow-y-auto flex-1">
                    {optionsList.map((tipo) => {
                      const selectedTypes = getMultiSelectSelectedTypes(
                        showUretanoMultiSelect.servicoKey!,
                        showUretanoMultiSelect.label!
                      );
